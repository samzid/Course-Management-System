...
{{-- {{dd($contactMessage_generalView)}} --}}
                @if ($contactMessage_generalView->type == 1)
                <a class="btn btn-primary" style="padding:5px 30px; font-size:15px;"
                href="{{ route('admin_message',['param'=>$contactMessage_generalView->id]) }}">Reply Message</a>
                @else
                <a class="btn btn-primary" style="padding:5px 30px; font-size:15px;"
                href="{{ route('user_message') }}">Reply Message</a>
                @endif
            ...
            <div class="col-sm-4 form-group">
                ...
                {{ $contactMessage_generalView->name }}
            </div>

            <div class="col-sm-4 form-group">
                <label for="email">Phone:</label>
                {{ $contactMessage_generalView->phone }}
            </div>

            <div class="col-sm-4 form-group">
                <label for="email">Email:</label>
                {{ $contactMessage_generalView->email }}
            </div>
            ...
            <div class="col-sm-9 form-group card" style="width: 500px; height:100px; margin-left:10px">
                ...
                {{ $contactMessage_generalView->message }}
